package edu.ipfw.collections;
import java.util.Scanner;

/**
 * Created by JasonWynkoop on 9/25/17.
 * This small example demonstrates how to use the IntArrayBag class.
 */
public class BagDemonstration {

    private static Scanner stdin = new Scanner(System.in);

    public static void main(String[] args){

        IntArrayBag ages = new IntArrayBag();
        getAges(ages);
        checkAges(ages);
        System.out.println("May your family live long and prosper!");
    }

    public static void getAges(IntArrayBag ages){
        /*
        The getAges methods prompts the user to type in the ages of family members.  These
          ages are read and places in the ages bag, stopping when the user types a negative
          number.  This demonstration does not worry about the possibility of running out of
          memory (therefore, an OutOfMemoryError is possible).
         */

        int userInput;              //An age from the user's family

        System.out.println("Type the ages of your family members.");
        System.out.println("Type a negative number at the end and press return.");
        userInput = stdin.nextInt();
        while (userInput >= 0){
            ages.add(userInput);
            userInput = stdin.nextInt();
        }
    }

    public static void checkAges(IntArrayBag ages){
        /*
        The checkAges method prompts the user to type in the ages of family members once
        again.  Each age is removed from the ages baf when it is type, stopping when the bag
        is empty
         */

        int userInput;              //An age from the user's family.

        System.out.println("Next age: ");
        userInput = stdin.nextInt();
        if (ages.countOcurences(userInput) == 0){
            System.out.println("No, that age does not occur!");
        }else{
            System.out.println("Yes, I've got that age and will remove it.");
            ages.remove(userInput);
        }
    }

}
